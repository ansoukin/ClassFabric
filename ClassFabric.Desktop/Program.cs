...
using ClassFabric.Platform.Windows;
using ClassFabric.Platform.Windows.Helpers;
using ClassFabric.Platform.Windows.Services;
#endif
#if Platforms_Linux
using ClassFabric.Platforms.Linux.Services;
#endif
#if Platforms_MacOs
using ClassFabric.Platforms.MacOs.Services;
#endif
...
class Program
{
    ...
    {
#if DEBUG
        if (!Debugger.IsAttached && Environment.GetEnvironmentVariable("ClassFabric_WaitDebuggers") == "1")
        {
        ...
        var buildApp = ClassFabric.Program.AppEntry(args);
