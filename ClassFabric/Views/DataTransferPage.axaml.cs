...
using ClassFabric.Models.External.ClassWidgets;
...
public partial class DataTransferPage : UserControl
{
    ...
    private async void SettingsExpanderImportFromClassFabric1_OnClick(object? sender, RoutedEventArgs e)
    {
        ViewModel.BrowseAction = BrowseClassFabricData;
        ViewModel.PerformImportAction = BeginPerformClassFabricImport;
        ViewModel.PageIndex = 1;
        ViewModel.ImportSourcePath = "";
        ViewModel.ImportDescription = "支持从 1.x 版本的 ClassFabric 导入课表、组件配置、自动化配置、应用设置、部分插件和主题等数据。";
        ViewModel.IsExport = false;
    }

    private async Task BrowseClassFabricData()
    {
        ...
        {
            Title = "选择先前版本的 ClassFabric 实例",
            FileTypeFilter =
            [
                new FilePickerFileType("ClassFabric 可执行文件")
                {
                    Patterns = ["ClassFabric.exe"]
                }
            ]
        }, topLevel);
    ...
    private async Task BeginPerformClassFabricImport()
    {
    ...
    private void ImportSettings(string root)
    {
        ...
        if (settings.WeatherIconId == "classfabric.weatherIcons.materialDesign")
        {
            settings.WeatherIconId = "classfabric.weatherIcons.lucide";
        }
    ...
    private void ImportProfile(string root)
    {
        ...
        {
            var config = ClassFabricV1ProfileTransferHelper.TransferClassFabricV1ProfileToClassFabricProfile(path);
    ...
    #region ClassFabric 2 / Import

    private void SettingsExpanderImportFromClassFabric2_OnClick(object? sender, RoutedEventArgs e)
    {
        ViewModel.BrowseAction = BrowseClassFabric2Data;
    ...
    private async Task BrowseClassFabric2Data()
    {
        var topLevel = TopLevel.GetTopLevel(this);
        if (topLevel == null)
        {
    ...
    private async Task BeginPerformClassFabric2Import()
    {
        var r = await new ContentDialog()
        {
            Title = "重启以继续",
    ...
    #region ClassFabric 2 / Export

    private void SettingsExpanderExportToClassFabric2_OnClick(object? sender, RoutedEventArgs e)
    {
        ViewModel.BrowseAction = BrowseClassFabric2ExportTargetAction;
    ...
    private async Task BrowseClassFabric2ExportTargetAction()
    {
        var topLevel = TopLevel.GetTopLevel(this);
        if (topLevel == null)
        {
        ...
        var file = await PlatformServices.FilePickerService.SaveFilePickerAsync(new FilePickerSaveOptions
        {
            ...
            SuggestedFileName = $"ClassFabric_data_{AppBase.AppVersion}_{DateTime.Now:yyyyMMdd_HHmmss}.cidata"
        }, topLevel);
        PopupHelper.RestoreAllPopups();
    ...
    private async Task PerformClassFabric2ExportAction()
    {
        if (string.IsNullOrWhiteSpace(ViewModel.ImportSourcePath))
        {
            return;
        ...
        try
        {
            ...
            var temp = Directory.CreateTempSubdirectory("ClassFabricDataExport").FullName;
    ...
    private void ImportClassWidgetsProfile(string root, IniData ini)
    {
        ...
        var profile =
            ClassWidgetsProfileTransferHelper.ConvertClassWidgets1ProfileToClassFabricProfile(Path.Combine(root,
    ...
    public async void ImportComplete(bool importV1)
    {
        if (importV1)
        {
            ...
            {
                Title = "正在升级插件",
                Body = "正在升级从 ClassFabric 1 导入的插件到兼容 ClassFabric 2 的版本，这可能需要一定的时间，应用将在升级完成后显示一条通知。部分插件可能暂不支持 ClassFabric 2。"
            });
