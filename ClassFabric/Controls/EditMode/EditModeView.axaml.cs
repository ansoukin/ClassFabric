...
using ClassFabric.Core.Services.Registry;
...
public partial class EditModeView : UserControl
{
    ...
    public EditModeView()
    {
        InitializeComponent();
        DataContext = this;
        ViewModel.TutorialService.Context["classfabric.mainWindow.editMode.mainDrawer.getState"] =
            () => ViewModel.MainDrawerState;
        ViewModel.TutorialService.Context["classfabric.mainWindow.editMode.secondaryDrawer.getState"] =
    ...
    protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
    {
        ...
        _mainDrawerStateObserver ??= ViewModel.ObservableForProperty(x => x.MainDrawerState)
            .Subscribe(_ =>
            {
                ...
                {
                    ViewModel.TutorialService.PushToNextSentenceByTag("classfabric.mainwindow.editMode.componentsLib.closed");
                }
                UpdateComponentSettingsTabIndex();
            });
    }
    ...
    public void OpenComponentsLibDrawer(IList<ComponentSettings>? target = null)
    {
        ...
        ViewModel.TutorialService.PushToNextSentenceByTag("classfabric.mainwindow.editMode.componentsLib.open");
    }
    ...
    public void ShowComponentSettings()
    {
        ...
        UpdateComponentSettingsTabIndex();
        ViewModel.TutorialService.PushToNextSentenceByTag("classfabric.mainwindow.editMode.componentSettings.open");
    }
    ...
    public void OpenChildComponents(ComponentSettings? settings, IReadOnlyList<ComponentSettings> stack, Point pos)
    {
        ...
        Dispatcher.UIThread.Post(() =>
        {
            ViewModel.TutorialService.PushToNextSentenceByTag(
                "classfabric.mainwindow.editMode.childrenComponent.open");
        });
    }
    ...
    private void ButtonOpenComponentLayoutsFolder_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        {
            FileName = Path.GetFullPath(ClassFabric.Services.ComponentsService.ComponentSettingsPath),
    ...
    private async void ButtonCreateComponentLayout_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        var path = Path.Combine(ClassFabric.Services.ComponentsService.ComponentSettingsPath,
            ViewModel.CreateProfileName + ".json");
        if (dialogResult != ContentDialogResult.Primary || File.Exists(path))
        {
            return;
        }
        ConfigureFileHelper.SaveConfig(path, ClassFabric.Services.ComponentsService.DefaultComponentProfile);
