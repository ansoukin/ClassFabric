...
[SpeechProviderInfo("classfabric.speech.gpt-sovits", "GPT-SoVITS")]
public class GptSoVitsService : ISpeechService
{
    ...
    private async Task<bool> GenerateSpeechAsync(string text, string filePath, CancellationToken cancellationToken)
    {
        ...
        if (settings.IsInternal && GptSovitsSecrets.IsSecretsFilled)
        {
            ...
            httpClient.DefaultRequestHeaders.Add("X-ClassFabric-ApiSignature", Convert.ToBase64String(Encoding.UTF8.GetBytes(sign)));
        }
        httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("ClassFabric", AppBase.AppVersion));
