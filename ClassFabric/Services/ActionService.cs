...
using static ClassFabric.Core.Abstractions.Services.IActionService;
...
public class ActionService : IActionService
{
    ...
    {
        // 1.7.107.0
        // 迁移应用设置行动 classfabric.settings
        {
            const string prefix = "classfabric.settings.";
            if (actionItem.Id.StartsWith(prefix))
            {
                ...
                actionItem.Id = "classfabric.settings";
    ...
    public ActionService(ILogger<ActionService> logger)
    {
        ...
        ActionMenuTree.Add(
            ...
            new ActionMenuTreeItem<RunActionSettings>("classfabric.os.run", "应用程序", "\uf4b1",
                s => s.RunType = RunActionSettings.RunActionRunType.Application),
            new ActionMenuTreeItem<RunActionSettings>("classfabric.os.run",
                OperatingSystem.IsWindows() ? "cmd 命令" : "终端命令",
                "\ue508",
                s => s.RunType = RunActionSettings.RunActionRunType.Command),
            new ActionMenuTreeItem<RunActionSettings>("classfabric.os.run", "文件", "\ue687",
                s => s.RunType = RunActionSettings.RunActionRunType.File),
            new ActionMenuTreeItem<RunActionSettings>("classfabric.os.run", "文件夹", "\ue875",
                s => s.RunType = RunActionSettings.RunActionRunType.Folder),
            new ActionMenuTreeItem<RunActionSettings>("classfabric.os.run", "Url 链接", "\ue905",
                s => s.RunType = RunActionSettings.RunActionRunType.Url)
        ));

        ActionMenuTree.Add(
            new ActionMenuTreeGroup("提醒", "\ue025",
            new ActionMenuTreeItem<NotificationActionSettings>("classfabric.showNotification", "显示提醒…", "\ue02f",
                s => s.IsWaitForCompleteEnabled = false),
            new ActionMenuTreeItem<NotificationActionSettings>("classfabric.showNotification", "显示提醒并等待…", "\ue02b",
                s => s.IsWaitForCompleteEnabled = true),
            new ActionMenuTreeItem<WeatherNotificationActionSettings>("classfabric.notification.weather", "三天天气预报", "\ue4dc",
                s => s.NotificationKind = 0),
            new ActionMenuTreeItem<WeatherNotificationActionSettings>("classfabric.notification.weather", "气象预警", "\uf431",
                s => s.NotificationKind = 1),
            new ActionMenuTreeItem<WeatherNotificationActionSettings>("classfabric.notification.weather", "逐小时天气预报", "\uf357",
                s => s.NotificationKind = 2)
        ));

        ActionMenuTree.Add(
            new ActionMenuTreeGroup("ClassFabric", "\ue454",
            new ActionMenuTreeItem("classfabric.app.quit", "退出 ClassFabric", "\ue0df"),
            new ActionMenuTreeItem("classfabric.app.restart", "重启 ClassFabric", "\ue0bd")
        ));

        return;

        static ActionMenuTreeItem<ModifyAppSettingsActionSettings> SettingItem(string displayName, string icon, string name) =>
            new("classfabric.settings", displayName, icon, s => s.Name = name);
    }

    [Obsolete("注意！行动 v2 注册方法已过时，请参阅 ClassFabric 开发文档进行迁移。")]
