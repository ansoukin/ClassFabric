...
using ClassFabric.Shared.Models.Management;
...
using ClassFabric.Shared.Protobuf.Client;
using ClassFabric.Shared.Protobuf.Service;
using FluentAvalonia.UI.Controls;
using Microsoft.Extensions.Logging;

using static ClassFabric.Shared.Helpers.ConfigureFileHelper;

namespace ClassFabric.Services.Management;
        ...
        try
        {
            ...
            if (Manifest.CoreVersion.Major != IAppHost.CoreVersion.Major)
            {
                await CommonTaskDialogs.ShowDialog("集控核心版本不兼容",
                    $"您正在核心版本主版本号（{IAppHost.CoreVersion}）与集控数据核心版本主版本号（{Manifest.CoreVersion}）不同的 ClassFabric 加入集控，这可能会导致您的数据损坏。请运行兼容贵组织集控服务的 ClassFabric 版本，或者联系您的管理员了解详细信息。");
                return false;
            }
