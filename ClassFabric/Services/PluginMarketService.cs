...
public class PluginMarketService : ObservableRecipient, IPluginMarketService
{
    ...
    {
        // 云端插件源已停用（任务A）：不再拉取/刷新云来源，保留本地插件读取逻辑。
        return;
            ...
            if (count > 0)
            {
                if (SettingsService.Settings.IsPluginsUpdateNotificationEnabled)
                {
                    await PlatformServices.DesktopToastService.ShowToastAsync(new DesktopToastContent()
                    {
                        ...
                        Activated = (_, _) => IAppHost.GetService<IUriNavigationService>().NavigateWrapped(new Uri("classisland://app/settings/classfabric.plugins"))
                    });
                }
    ...
    public IEnumerable<PluginIndexInfo> GetIndexInfos()
    {
        ...
        const string repo = "https://get.classisland.tech/d/ClassIsland-Ningbo-S3/classfabric/plugin/index.zip?time={time}";
